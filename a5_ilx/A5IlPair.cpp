	...
	mUsable(false)
{
    ...
    if (kernel == NULL) {
		printf ( " [E] A5Il:   [%i] Could not load optimized kernel\r\n", mDevNo);
		return;
    }

    if (calclCompile(&mObjectDouble, CAL_LANGUAGE_IL, (const CALchar*)kernel,
                     mDev->getDeviceInfo()->target) != CAL_RESULT_OK) {
		printf ( " [E] A5Il:   [%i] Compilation failed.\r\n", mDevNo);
		printf ( " [E] A5Il:   [%i]   Reason: %s\r\n", mDevNo, calclGetErrorString());
		printf ( " [E] A5Il:   [%i]   Your card might be too old for this code.\r\n", mDevNo);
		return;
    }
    freeKernel(kernel);

    if (calclLink (&mImageDouble, &mObjectDouble, 1) != CAL_RESULT_OK) {
		printf ( " [E] A5Il:   [%i] Link failed.\r\n", mDevNo);
		printf ( " [E] A5Il:   [%i]   Reason: %s\r\n", mDevNo, calclGetErrorString());
		return;
    }
#if DISASSEMBLE
    ...
    if (mModuleDouble==0) {
		printf ( " [E] A5Il:   [%i] Could not create module.\r\n", mDevNo);
		printf ( " [E] A5Il:   [%i]   Reason: %s\r\n", mDevNo, calclGetErrorString());
		return;
    }
    if (!mModuleDouble->Load(mImageDouble)) {
		printf ( " [E] A5Il:   [%i] Could not load module image.\r\n", mDevNo);
		printf ( " [E] A5Il:   [%i]   Reason: %s\r\n", mDevNo, calclGetErrorString());
		return;
    }
	...
	printf(" [x] A5Il:   [%i] Using %i threads\n", mDevNo, mNum );
